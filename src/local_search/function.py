...
class Function:
    ...
    def __call__(self, n: node.Node) -> float:
        ...
        Raises:
            AssertionError: If the node is outside the defined bounds.
        """
        assert self._x_bounds[0] <= n.x <= self._x_bounds[1], (
            f"x={n.x} is outside bounds {self._x_bounds}"
        )
        assert self._y_bounds[0] <= n.y <= self._y_bounds[1], (
            f"y={n.y} is outside bounds {self._y_bounds}"
        )
